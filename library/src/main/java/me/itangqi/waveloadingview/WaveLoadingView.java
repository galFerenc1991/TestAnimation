...
public class WaveLoadingView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        /////////////////
        waveBackgroundPaint = new Paint();
        waveBackgroundPaint.setColor(Color.argb(255, 14, 16, 49));
        ///////////////////////
        attributes.recycle();
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        // Draw Wave.
        // Modify waveBackgroundPaint shader according to mShowWave state.
        if (mWaveShader != null) {
            // First call after mShowWave, assign it to our waveBackgroundPaint.
            ...
            // Get borderWidth.
            borderWidth = mBorderPaint.getStrokeWidth();
                ...
                case 1:
                    if (borderWidth > 0) {
                        canvas.drawCircle(getWidth() / 2f, getHeight() / 2f,
                                getWidth()/ 4f + borderWidth/2, mBorderPaint);
                        Log.d("Width", "" + getWidth());
                        Log.d("Height", "" + getHeight());
                    }

                    float radius = getWidth() / 4f;
                    canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, getWidth()/4f, waveBackgroundPaint);
        ...
        if (bitmapBuffer == null || haveBoundsChanged()) {
            ...
            if(width > 0 && height > 0) {
                ...
                float[] waveY = new float[endX];
                    //0.3f
                wavePaint.setColor(adjustAlpha(mWaveColor, 0f));
                for (int beginX = 0; beginX < endX; beginX++) {
                    double wx = beginX * defaultAngularFrequency;
                    float beginY = (float) (mDefaultWaterLevel + defaultAmplitude * Math.sin(wx));
                    wavePaint.setShader(new LinearGradient((float) beginX, beginY,
                            (float) beginX, (float) endY, Color.argb(255, 1, 226, 255), Color.argb(255, 15, 132, 160), Shader.TileMode.MIRROR));
